...
def do_query(archives, config_file=None, logger=None, context=None):
    ...
    matching_pages = filter_pages.map(
        lambda title_page: (
            title_page[0],
            get_sentences_list_matches_per_page(title_page[1], keysentences),
        )
    )

    # [[(year, keysentence), 1) ((year, keysentence), 1) ] ...]
    matching_sentences = matching_pages.flatMap(
        lambda title_sentence: [
            ((title_sentence[0], sentence), 1)
    ...
    result = (
        ...
        .groupByKey()
        .map(
            lambda title_sentencecount: (
                title_sentencecount[0],
                list(title_sentencecount[1]),
            )
        )
        .collect()
    )
